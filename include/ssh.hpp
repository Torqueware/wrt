...
/* This class describes a SSH Exception object. This object can be 
...
public:
   ...
   *   o. SSH_FATAL Fatal error happened (not recoverable)
   *   o. SSH_REQUEST_DENIED Request was denied by remote host
   */
  int getCode() {
    return code;
  }

  /* Returns a pointer to the error message of the last exception */ 
  std::string getError() {
    return description;
  }
...
public:
  ...
  int userauthPublickeyAuto(void) {
    int ret = 0;//ssh_userauth_publickey_auto(c_session, NULL, NULL);
    ssh_throw(ret);
    return ret;
  }
